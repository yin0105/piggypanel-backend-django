...
from .models import contacts, contacts_fields, contacts_addedit_layout, contacts_browse_layout
from .forms import ContactsLayoutFieldForm
import json
import pickle
...
class AdminContactForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        fields_properties = contacts_fields.objects.all()
        ...
        for p in fields_properties:
            ...
            if((p.field_type == "datetime") and bool(p.field_isreadonly) != True):                
                self.base_fields[str(p.field_api_name)] = forms.DateTimeField(widget=widgets.AdminDateTimeWidget(),label=str(p.field_label), max_length=p.field_size, initial=value_data, required=mandatory)
                self.fields[str(p.field_api_name)] = forms.DateTimeField(widget=widgets.AdminDateTimeWidget(),label=str(p.field_label), max_length=p.field_size, initial=value_data, required=mandatory)  
...
class DuplicatePhoneRecordsFilter(SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == 'phoneduplicates':
            ...
            dataset = contacts.objects.values('data__Phone').exclude(data__Phone__exact='').exclude(data__Phone = None).annotate(group_phones = Count('data__Phone')).filter(group_phones__gte=2).values_list('data__Phone', flat=True)

            queryset = contacts.objects.filter(data__Phone__in = dataset).order_by('data__Phone')

            return queryset
        if self.value() == 'mobileduplicates':
            
            dataset = contacts.objects.values('data__Mobile').exclude(data__Mobile__exact='').exclude(data__Mobile = None).annotate(group_phones = Count('data__Mobile')).filter(group_phones__gte=2).values_list('data__Mobile', flat=True)

            queryset = contacts.objects.filter(data__Mobile__in = dataset).order_by('data__Mobile')

            return queryset
        if self.value() == 'phonemobileduplicates':
            
            mobiledataset = contacts.objects.values('data__Mobile').exclude(data__Mobile__exact='').exclude(data__Mobile = None).annotate(group_mobiles = Count('data__Mobile')).filter(group_mobiles__gte=2).values('data__Phone').exclude(data__Phone__exact='').exclude(data__Phone = None).annotate(group_phones = Count('data__Phone')).filter(group_phones__gte=2).values_list('data__Mobile', flat=True)

            queryset = contacts.objects.filter(data__Mobile__in = mobiledataset).order_by('data__Mobile')

            return queryset
        if self.value() == 'anyphonemobileduplicates':
            
            mobiledataset = contacts.objects.values('data__Mobile').exclude(data__Mobile__exact='').exclude(data__Mobile = None).values_list('data__Mobile', flat=True)

            phonedataset = contacts.objects.values('data__Phone').exclude(data__Phone__exact='').exclude(data__Phone = None).values_list('data__Phone', flat=True)

            queryset = contacts.objects.filter(Q(data__Phone__in = mobiledataset) | Q(data__Mobile__in = phonedataset)).order_by('data__Phone', 'data__Mobile')
...
class ContactsAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        contact_fields = contacts_fields.objects.all()
...
class ContactsLayoutAdmin(admin.ModelAdmin):
    form = ContactsLayoutFieldForm
...
class ContactsBrowseLayoutAdmin(admin.ModelAdmin):
    list_display = ['layout_col', 'layout_col_width', 'layout_field', 'layout_field_searchable', 'layout_field_sortable']
    ordering = ['layout_col']

admin.site.register(contacts, ContactsAdmin)
admin.site.register(contacts_fields, ContactFieldsAdmin)
admin.site.register(contacts_addedit_layout, ContactsLayoutAdmin)
admin.site.register(contacts_browse_layout, ContactsBrowseLayoutAdmin)